...
class NoteModel {
    ...
    static async create(note) {
        ...
        try {
            ...
            const [userRows] = await database_1.db.execute('SELECT id FROM users WHERE id = ?', [userId]);
            ...
            const [result] = await database_1.db.execute('INSERT INTO notes (user_id, title, content, is_pinned, is_password_protected, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())', [
                userId,
                note.title,
                note.content || '',
                note.isPinned ? 1 : 0,
    ...
    static async findById(id) {
        const [rows] = await database_1.db.execute('SELECT * FROM notes WHERE id = ?', [id]);
        if (!rows.length)
    ...
    static async findByUser(userId) {
        ...
        try {
            ...
            // Find all notes where user is owner OR collaborator
         ...
         ORDER BY n.is_pinned DESC, n.updated_at DESC`, [userId, userId]);
            ...
            if (rows.length === 0) {
                ...
                try {
                    ...
                    return newRows.map(this.mapRow);
                }
                catch (error) {
    ...
    static async findWithCollaborators(noteId, userId) {
        try {
            // Check if the user has access to this note (owner or collaborator)
            const [accessRows] = await database_1.db.execute(`SELECT 1 FROM (
          SELECT user_id FROM notes WHERE id = ?
          UNION
          SELECT user_id FROM note_shares WHERE note_id = ?
         ) as access
         WHERE access.user_id = ?`, [noteId, noteId, userId]);
            if (!accessRows || !accessRows.length) {
                return null; // User doesn't have access
            }
            // Get note data
            const [noteRows] = await database_1.db.execute('SELECT * FROM notes WHERE id = ?', [noteId]);
            if (!noteRows || !noteRows.length) {
                return null; // Note doesn't exist
            }
            const note = this.mapRow(noteRows[0]);
            // Get collaborators
            const [collaboratorRows] = await database_1.db.execute(`SELECT nc.role, u.id as userId, u.email, u.display_name as displayName
         FROM note_shares nc
    ...
    static async update(id, userId, updates) {
        ...
        await database_1.db.execute(`UPDATE notes SET ${fields.join(', ')}, updated_at = NOW() WHERE id = ? AND user_id = ?`, values);
        return this.findById(id);
    }
    static async delete(id, userId) {
        await database_1.db.execute('DELETE FROM notes WHERE id = ? AND user_id = ?', [id, userId]);
    }
    static async pin(id, userId, pin) {
        await database_1.db.execute('UPDATE notes SET is_pinned = ? WHERE id = ? AND user_id = ?', [pin ? 1 : 0, id, userId]);
        return this.findById(id);
    }
    static async setPassword(id, userId, password) {
        try {
            // Get note to confirm ownership
            const [rows] = await database_1.db.execute('SELECT * FROM notes WHERE id = ? AND user_id = ?', [id, userId]);
            if (!rows || !rows.length) {
                throw new errorHandler_1.AppError('Note not found or you do not have permission', 404);
            }
            // If removing password protection
            if (password === null) {
                await database_1.db.execute('UPDATE notes SET is_password_protected = 0, password_hash = NULL, updated_at = NOW() WHERE id = ?', [id]);
                return this.findById(id);
            }
            // Hash the password
            const passwordHash = await bcrypt.hash(password, 10);
            // Update the note with password protection
            await database_1.db.execute('UPDATE notes SET is_password_protected = 1, password_hash = ?, updated_at = NOW() WHERE id = ?', [passwordHash, id]);
            return this.findById(id);
        }
        catch (error) {
    ...
    static async verifyPassword(id, password) {
        try {
            const [rows] = await database_1.db.execute('SELECT password_hash FROM notes WHERE id = ? AND is_password_protected = 1', [id]);
    ...
    static async addCollaborator(noteId, ownerId, collaboratorEmail, role) {
        try {
            // Check note ownership
            const [ownerRows] = await database_1.db.execute('SELECT * FROM notes WHERE id = ? AND user_id = ?', [noteId, ownerId]);
            if (!ownerRows || !ownerRows.length) {
                throw new errorHandler_1.AppError('Note not found or you are not the owner', 403);
            }
            // Find the user by email
            const [userRows] = await database_1.db.execute('SELECT id FROM users WHERE email = ?', [collaboratorEmail]);
            if (!userRows || !userRows.length) {
            ...
            if (collaboratorId === ownerId) {
                throw new errorHandler_1.AppError('You cannot add yourself as a collaborator', 400);
            }
            // Check if already a collaborator
            const [existingRows] = await database_1.db.execute('SELECT * FROM note_shares WHERE note_id = ? AND user_id = ?', [noteId, collaboratorId]);
            if (existingRows && existingRows.length > 0) {
                // Update role if different
                if (existingRows[0].role !== role) {
                    await database_1.db.execute('UPDATE note_shares SET role = ? WHERE note_id = ? AND user_id = ?', [role, noteId, collaboratorId]);
                }
            }
            else {
                // Add new collaborator
                await database_1.db.execute('INSERT INTO note_shares (note_id, user_id, role) VALUES (?, ?, ?)', [noteId, collaboratorId, role]);
            }
            return true;
        }
        catch (error) {
    ...
    static async removeCollaborator(noteId, ownerId, collaboratorId) {
        try {
            // Check note ownership
            const [ownerRows] = await database_1.db.execute('SELECT * FROM notes WHERE id = ? AND user_id = ?', [noteId, ownerId]);
            if (!ownerRows || !ownerRows.length) {
            ...
            // Remove collaborator
            await database_1.db.execute('DELETE FROM note_shares WHERE note_id = ? AND user_id = ?', [noteId, collaboratorId]);
            return true;
        }
        catch (error) {
